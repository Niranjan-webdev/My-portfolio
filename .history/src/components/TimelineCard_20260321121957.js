    ...
    const Card = (
      <div className="bg-white rounded-2xl p-7 shadow-xl hover:shadow-2xl transition-shadow duration-200">
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap mb-4">
          <h3 className="text-[#2eb89c] font-bold text-2xl leading-snug m-0">
            {item.title}
          </h3>
          <span className="bg-gray-100 text-gray-800 text-xs font-medium px-3 py-2 rounded-full whitespace-nowrap shrink-0">
            {item.date}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2.5 mb-4">
          ...
          <span className="text-xl font-bold text-gray-900">{item.company}</span>
        </div>
  
        {/* Bullets */}
        <ul className="flex flex-col items-center justify-center gap-2.5 p-0 m-0 list-none">
          {item.bullets.map((b, i) => (
            <li key={i} className="flex gap-2.5 text-base font-[600] text-gray-800 leading-relaxed">
